...
public class PathfindingDebugStepVisual : MonoBehaviour
{
    ...
    public void TakeSnapshot(Grid<PathNode> grid, PathNode current, List<PathNode> openList, List<PathNode> closedList)
    {
        ...
        for (int x = 0; x < grid.GetWidth(); x++)
        {
            for (int z = 0; z < grid.GetHeight(); z++)
            {
                ...
                Vector3 gridPosition = new Vector3(pathNode.x, 0, pathNode.z) * grid.GetCellSize() + Vector3.one * grid.GetCellSize() * .5f;
                ...
                gridSnapshotAction.AddAction(() => {
                    Transform visualNode = visualNodeArray[tmpX, tmpZ];
    ...
    private Transform CreateVisualNode(Vector3 position)
    {
        ...
        Transform visualNodeTransform = Instantiate(pfPathfindingDebugStepVisualNode, new Vector3(position.x, 0, position.z), Quaternion.Euler(new Vector3(90f,0f,0f)));
